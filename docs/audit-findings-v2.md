...
| 8 | `/api/notify-account` accepts unauthenticated POST | Medium | Mitigated post-cutover | See finding #10. |
| 9 | `/api/notify-account` `accessUrl` allowlist `/\.vercel\.app$/` admits ANY `*.vercel.app` deployment | Medium | **Fixed post-cutover** | Original Task 7a code-review flagged this; shipped verbatim per plan, tightened in follow-up commit after `main` cutover. Allowlist now `/^weeks-iot(-[\w-]+)*\.vercel\.app$/` — matches only weeks-iot project deploys. `attacker.vercel.app` rejects with 400. Verified locally with curl. |
| 10 | `/api/notify-account` could relay outbound email at attacker-controlled rate | Medium | **Mitigated post-cutover** | Added `Origin` header gate: requests with an `Origin` not in `ALLOWED_HOSTS` return 403. Browser drive-by abuse (most realistic spam vector) is blocked. Requests without an `Origin` header (server-to-server, healthchecks) still pass — the design treats Origin as a soft gate, not auth. Determined attackers using curl can fake the header. v3 follow-ups: per-IP rate limit via `@vercel/kv`, or session-cookie gate from a prior PIN entry. |
